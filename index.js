 ...
 * @returns {Promise<{statusCode: number, statusMessage: string, headers: {[key: string]: string}, content: any}>}
 */
async function requestWithRetryPolicyAsync(req) {
  let res;
  for (let i = 0; i < RETRY_POLICY.length; i++) {
    // console.error("request with retry", i);
    if (0 < i) {
      await timeout(variance(1000 * RETRY_POLICY[i]));
    }
    try {
...
requestWithRetryPolicyAsync.requestAsync = requestAsync;

module.exports = requestWithRetryPolicyAsync;
